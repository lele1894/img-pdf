...
class InteractiveAdRemoverGUI:
    ...
    def create_widgets(self):
        ...
        # 白边阈值选项
        margin_threshold_frame = tk.Frame(options_frame, bg="#ecf0f1")
        margin_threshold_frame.pack(fill="x", padx=8, pady=2)

        margin_threshold_label = tk.Label(
            margin_threshold_frame,
            text="白边阈值:",
            font=("Arial", 9),
            bg="#ecf0f1"
        )
        margin_threshold_label.pack(side="left")

        self.margin_threshold_var = tk.IntVar(value=200)
        margin_threshold_scale = tk.Scale(
            margin_threshold_frame,
            from_=180,
            to=255,
            orient="horizontal",
            variable=self.margin_threshold_var,
            length=150,
            bg="#ecf0f1"
        )
        margin_threshold_scale.pack(side="left", padx=5)

        # 保留白边选项
        margin_padding_frame = tk.Frame(options_frame, bg="#ecf0f1")
        margin_padding_frame.pack(fill="x", padx=8, pady=2)

        margin_padding_label = tk.Label(
            margin_padding_frame,
            text="保留白边:",
            font=("Arial", 9),
            bg="#ecf0f1"
        )
        margin_padding_label.pack(side="left")

        self.margin_padding_var = tk.IntVar(value=40)
        margin_padding_scale = tk.Scale(
            margin_padding_frame,
            from_=0,
            to=100,
            orient="horizontal",
        ...
        margin_padding_scale.pack(side="left", padx=5)
...
☑ 去除保留区域外的白边
   ...
   • 白边阈值:控制白色检测的灵敏度 (180-255,默认200)
   • 保留白边:控制保留的白边宽度 (0-100像素,默认20)
   • 如需精确保留框选区域,建议取消勾选此选项
    ...
    def process_pdf(self):
        ...
        try:
            ...
            remover = KeepRegionRemover(
                ...
                self.margin_threshold_var.get()
            )
            remover.margin_padding = self.margin_padding_var.get()
...
class KeepRegionRemover:
    ...
    def __init__(self, keep_regions, remove_margins=True, margin_threshold=200):
        """
        初始化保留区域处理器
        ...
        Args:
            ...
            margin_threshold: 白边检测阈值 (180-255)
        """
        self.keep_regions = keep_regions
        self.remove_margins = remove_margins
        self.margin_threshold = margin_threshold
        self.margin_padding = 0  # 保留的白边像素数
    
    def process_pdf(self, pdf_path, output_pdf_path=None, margin_threshold=None):
        """
        处理PDF文件,保留指定区域并去除白边
        ...
        Args:
            ...
            margin_threshold: 白边检测阈值 (如果为None,使用初始化时的值)
        ...
        for i, page in enumerate(pdf_document):
            ...
            if self.remove_margins:
                threshold = margin_threshold if margin_threshold is not None else self.margin_threshold
                image = self.remove_white_margins(image, threshold, padding=self.margin_padding)
    ...
    def remove_white_margins(self, image, threshold=None, padding=0):
        """
        去除图片四周的白边

        Args:
            image: OpenCV图像对象
            threshold: 白边检测阈值 (默认使用初始化时的值)
            padding: 保留的白边像素数 (正值表示保留额外白边，负值表示向内裁剪)
        ...
        if padding > 0:
            ...
            # 向内裁剪更多
            x = min(x - padding, image.shape[1] - 1)
            y = min(y - padding, image.shape[0] - 1)
            w = max(w + 2 * padding, 10)
            h = max(h + 2 * padding, 10)
